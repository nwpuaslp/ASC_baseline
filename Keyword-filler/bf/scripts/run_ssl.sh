...
function usage {
  ...
  echo "  --backend      <ml|srp|music>              # backend algorithm to choose, (default=$backend)"
...
$cmd JOB=1:$nj $exp_dir/log/do_ssl.JOB.log \
  /home/environment/jwu/miniconda3/envs/jwu/bin/python ./scripts/sptk/do_ssl.py $ssl_opts \
  --backend $backend \
  --doa-range $doa_range \
  --output $output \
  $exp_dir/wav.JOB.scp \
