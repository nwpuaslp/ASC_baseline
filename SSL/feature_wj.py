...
class STFT(STFTBase):
    ...
    def forward(self, x, cplx=False):
        ...
        if cplx:
            return r, i
        m = th.clamp(r**2 + i**2, min=EPSILON)**0.5
        p = th.atan2(i+EPSILON, r+EPSILON)
        return m, p
...
class FeatureExtractor(nn.Module):
    ...
    def __init__(self,
        ...
        if ang_index:
            self.ang_extractor = AngleFeature(num_bins=num_bins,
                                              num_doas=1,
                                              af_index=ang_index)
    ...
    def _check_args(self, x, doa):
        ...
        if isinstance(doa, Sequence) and len(doa) != self.num_spks:
            raise RuntimeError("Number of DoA do not match the "
                               "speaker number: {:d} vs {:d}".format(
                                   len(doa), self.num_spks))
    ...
    def compute_spatial(self, x, pha=None, doa=None):
        """
    ...
    def forward(self, x, doa=None, ref_channel=None):
        """
        args
            x: N x C x S (multi-channel) or N x S (single channel)
